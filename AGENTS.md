...
تطبيق ويب عربي (RTL) لميزة «Verify»: رحلة بيع/شراء سيارات آمنة داخل منصة حراج.
**الهوية (2026-07-10):** الاسم التجاري «Verify» وشعار درع بعلامة صح (`src/components/verify-logo.tsx` — يرث currentColor). كلمة «حراج» المفردة باقية في نصوص النظام المقدسة (مراقبة من حراج، عمولة حراج...) بانتظار قرار المنتج.
...
- **الأسلوب:** هيكل حراج المألوف + فخامة هادئة: زوايا 16px+، ظلال ناعمة (`shadow-soft`/`shadow-lift`)، glassmorphism (`backdrop-blur` + شفافية)، حركات framer-motion قصيرة (150–400ms) مع احترام `prefers-reduced-motion`.
